...
//		$(this).append('<div class="scrubbar"></div>');
		...
		$('.scrubbar .ui-slider-handle:nth-child(1)', this).addClass('clip_in').bind('dblclick', function() { 
			e = qt($('embed', that));
			e.SetTime($('.scrubbar', that).slider('values', 0) * e.GetTimeScale());
		});
		...
		$('.scrubbar .ui-slider-handle:nth-child(3)', this).addClass('clip_out').bind('dblclick', function() { 
			e = qt($('embed', that));
			e.SetTime($('.scrubbar', that).slider('values', 2) * e.GetTimeScale());
		});
        ...
        $('.scrubbar', this).append('<div class="highlight"></div>');
        $('.scrubbar', this).append('<div class="range"></div>');
        
        	$('.scrubbar', this).slider_highlight({event: 'slider:highlight', selector: '.highlight'});
        	$('.scrubbar', this).slider_highlight({event: 'slider:range', selector: '.range', autohide: false});
		}
