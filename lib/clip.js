...
//		$('input.clip_out', options.tools).change(function() { video.duration = $.media.convertTimestamp($(this).val()) - video.start;});	
	...
	//	});
		...
		$('input.clip_in', options.tools).add('input.clip_out', options.tools).change(function() { 
		i = $(this).parent().find('input.clip_in').val();
		o = $(this).parent().find('input.clip_out').val();
		$(options.tools).parent().find('.scrubbar').trigger('slider:range', {begin: $.media.convertTimestamp(i), end: $.media.convertTimestamp(o)}); });
	},
